...
class SlingshotScanner:
    ...
    def analyze(self, close_prices):
        ...
        curr = close_prices[-1]
        buffer = ma * 0.00025 # 0.025% buffer (approx $25 at $100k)
        
        # Exit Condition (Standard Cross)
        ...
        # Entry Condition (Requires Buffer + Previous Close confirmation)
        p1, p2 = close_prices[-2], close_prices[-3]
        if curr > ma + buffer and (p1 < ma or p2 < ma): self.triggered_signal = "UP (Reclaim)"; return self.triggered_signal
        if curr < ma - buffer and (p1 > ma or p2 > ma): self.triggered_signal = "DOWN (Break)"; return self.triggered_signal
        return "WAIT"
...
class PolyOddsTrendScanner:
    ...
    def analyze(self, current_price, open_price, poly_up, poly_down, trend_prob):
        ...
        # CHANGED: Increased Poly threshold to 0.60 and Trend to 0.60
        if poly_up >= 0.60 and poly_down <= 0.40 and trend_prob >= 0.6 and current_price > open_price:
            self.triggered_signal = 'UP (Poly+Trend)'; return self.triggered_signal
            
        # CHANGED: Increased Poly threshold to 0.60 and Trend to 0.40
        if poly_down >= 0.60 and poly_up <= 0.40 and trend_prob <= 0.4 and current_price < open_price:
...
class TrendOddsScanner:
    ...
    def analyze(self, trend_prob, odds, curr, open_):
        ...
        # CHANGED: Stricter Reset (Exit if trend flips even slightly)
        if self.triggered == "UP (Trend+Odds)" and curr < open_: self.triggered = None
        elif self.triggered == "DOWN (Trend+Odds)" and curr > open_: self.triggered = None
        
        if self.triggered: return self.triggered
        
        # CHANGED: Require higher trend probability (0.60 instead of 0.55)
        trend_supports_up = trend_prob > 0.60 
        trend_supports_down = trend_prob < 0.40
        
        high_odds = odds >= 3
        
        if is_up:
            # CHANGED: Only enter High Odds if trend also supports it
            if (high_odds and trend_supports_up) or (trend_supports_up and odds >= 2):
                self.triggered = "UP (Trend+Odds)"; return self.triggered
        else:
            if (high_odds and trend_supports_down) or (trend_supports_down and odds >= 2):
                self.triggered = "DOWN (Trend+Odds)"; return self.triggered
        return "WAIT"

class ReboundScanner:
    def __init__(self): 
        self.high_up = 0.0; self.high_down = 0.0
        self.dip_up = False; self.dip_down = False
        self.triggered = None
    ...
    def analyze(self, up_p, down_p):
        # Track Highs
        self.high_up = max(self.high_up, up_p)
        self.high_down = max(self.high_down, down_p)
        
        if self.triggered: return self.triggered
        
        # --- UP SIDE LOGIC ---
        if self.high_up >= 0.70:
            if up_p <= 0.60: self.dip_up = True # Confirm Dip
            
            # Require Bounce to 62c+ AFTER dipping
            if self.dip_up and up_p >= 0.62:
                self.triggered = "UP (Rebound)"
                return self.triggered

        # --- DOWN SIDE LOGIC ---
        if self.high_down >= 0.70:
            if down_p <= 0.60: self.dip_down = True # Confirm Dip
            
            # Require Bounce to 62c+ AFTER dipping
            if self.dip_down and down_p >= 0.62:
                self.triggered = "DOWN (Rebound)"
                return self.triggered

        return "WAIT"

def calculate_master_signal(sling, poly, cobra, flag, to_sig, rebound, up_price):
    score = 0
...
class SniperApp(App):
    ...
    def __init__(self, sim_broker, live_broker, start_live_mode=False):
        ...
        self.rebound = ReboundScanner()
        self.window_bets = {} # Map: bet_id -> {"entry_price": float, "side": str} 
        self.app_start_time = time.time()
        self.time_rem_str = "05:00"
        
        # Risk Management State
        self.dynamic_risk_cap = 0.0
        self.max_session_cap = 0.0 # Hard Cap
        self.risk_cap_initialized = False
        self.live_spent_this_window = 0.0
        
        # Hybrid Price State
        self.last_cl_fetch = 0
        self.price_offset = 0
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Horizontal(
            Container(
                ...
                Label("Poly Odds: -/5", id="p_btc_odds", classes="price_sub"),
                Label("Diff vs Open: $0", id="p_btc_diff", classes="price_sub"),
                Label("Cobra: WAIT", id="p_cobra", classes="price_sub"),
                Label("Trend Str: -", id="p_btc_trend", classes="price_sub"),
                Label("Range 5m: $0", id="p_btc_rng", classes="price_sub"),
                Label("Flag: WAIT", id="p_flag", classes="price_sub"),
                Label("TO: WAIT", id="p_to", classes="price_sub"), 
                Label("Rebound: WAIT", id="p_rebound", classes="price_sub"), 
        ...
        yield Container(
            ...
            Input(placeholder="TP %", value="33", id="inp_tp"),
            Label("SL %:", classes="lbl_sm"),
            Input(placeholder="SL %", value="25", id="inp_sl"),
            classes="input_group"
        )
        ...
        yield Container(
            ...
            Input(placeholder="Min Diff $", value="10", id="inp_min_diff"),
            Label("Min Price:", classes="lbl_sm"),
            Input(placeholder="Min Price", value="0.50", id="inp_min_price"),
        ...
        yield Vertical(
            Horizontal(
                ...
                Checkbox("Rebound", value=True, id="cb_rebound"),
                classes="algo_row"
            ),
            # --- SETTINGS ROW ---
            Horizontal(
                ...
                Checkbox("Take Profit", value=True, id="cb_tp_active"),
    ...
    def on_live_toggle(self, event: Checkbox.Changed):
        if event.value: 
            ...
            for cid in ["#cb_sling", "#cb_poly", "#cb_cobra", "#cb_flag", "#cb_to", "#cb_rebound"]:
    ...
    def on_any_checkbox(self, event: Checkbox.Changed):
        ...
        if self.query_one("#cb_rebound").value: algos.append("Rebound")
        ...
        if self.query_one("#cb_tp_active").value: settings.append("TP_Active")
        if self.query_one("#cb_whale").value: settings.append("WhaleProtect")
    ...
    async def fetch_market_loop(self):
        try:
            ...
            if self.market_data["start_ts"] != 0 and ts_start != self.market_data["start_ts"]:
                ...
                # --- SETTLEMENT LOGIC ---
                is_live = self.query_one("#cb_live").value
                if self.risk_cap_initialized and not is_live: self._add_risk_revenue(sim_payout)
                ...
                self.cobra.reset(); self.bullflag.reset(); self.trend_odds.reset(); self.rebound.reset()
            ...
            def get_data(app_ref, s_up, s_down):
                ...
                d = {"curr":0,"open":0,"rng":0,"odds":1,"t_score":3,"t_prob":0.5,
                     "sling":"WAIT","poly":"WAIT","cobra":"WAIT","flag":"WAIT", "to_sig":"WAIT", "rebound":"WAIT"}
                
                # --- HYBRID PRICE FETCHING ---
                # --- CHAINLINK DIRECT PRICE FETCHING ---
                try: 
                    d["curr"] = float(app_ref.chainlink_contract.functions.latestAnswer().call()) / 10**8
                except: 
                    d["curr"] = app_ref.market_data["btc_price"] # Fallback to last known

                try:
                    k = s.get("https://api.binance.com/api/v3/klines", params={"symbol":"BTCUSDC","interval":"1m","limit":90}).json()
                    # Calculate Interpolated Range
                    start_ts = app_ref.market_data["start_ts"]
                    elapsed_calc = time.time() - start_ts
                    rem_exact = max(0.5, (300 - elapsed_calc) / 60.0)
                    
                    # Logic: e.g. 4.5m -> 50% of 4m + 50% of 5m
                    w_low = int(rem_exact); w_high = w_low + 1
                    if w_low < 1: w_low = 1; w_high = 2 # Min 1m window
                    
                    hist = k[:-1]; closes = [float(x[4]) for x in k]
                    
                    def calc_rng(mins):
                        if len(hist) <= mins: return 0
                        rec = hist[-60:]
                        r_list = [max(float(x[2]) for x in rec[i:i+mins]) - min(float(x[3]) for x in rec[i:i+mins]) for i in range(len(rec)-mins+1)]
                        return sum(r_list)/len(r_list) if r_list else 0

                    rng_l = calc_rng(w_low)
                    rng_h = calc_rng(w_high)
                    fraction = rem_exact - int(rem_exact)
                    
                    d["rng"] = rng_l * (1 - fraction) + rng_h * fraction
                    d["rem_display"] = rem_exact
                    
                    if int(k[-1][0]) == start_ts * 1000: d["open"] = float(k[-1][1]) 
                    for x in k: 
                        if int(x[0]) == start_ts * 1000: d["open"] = float(x[1]); break
                    if d["open"] == 0 and k: d["open"] = float(k[-1][1])

                    d["sling"] = app_ref.slingshot.analyze(closes)
                    d["cobra"] = app_ref.cobra.analyze(closes, d["curr"], elapsed_calc)
                ...
                try:
                    ...
                    up_ask = float(s.get("https://clob.polymarket.com/price", params={"token_id":uid,"side":"sell"}).json().get("price",0))
                    down_ask = float(s.get("https://clob.polymarket.com/price", params={"token_id":did,"side":"sell"}).json().get("price",0))
                    up_bid = float(s.get("https://clob.polymarket.com/price", params={"token_id":uid,"side":"buy"}).json().get("price",0))
                    down_bid = float(s.get("https://clob.polymarket.com/price", params={"token_id":did,"side":"buy"}).json().get("price",0))
                    ...
                    d["rebound"] = app_ref.rebound.analyze(up_ask, down_ask)
            ...
            d = res[6]
            score, status = calculate_master_signal(d["sling"], d["poly"], d["cobra"], d["flag"], d["to_sig"], d["rebound"], res[2])
            ...
            active_scanners = {
                ...
                "Flag": d["flag"], "TrendOdds": d["to_sig"], "Rebound": d["rebound"]
            }
            cb_map = {
                "Sling": "#cb_sling", "Poly": "#cb_poly", "Cobra": "#cb_cobra",
                "Flag": "#cb_flag", "TrendOdds": "#cb_to", "Rebound": "#cb_rebound"
            }
            ...
            self.query_one("#p_btc_open").update(f"Open: ${md['btc_open']:,.0f}")
            self.query_one("#p_btc_rng").update(f"Range {md.get('btc_rem_disp', 5.0):.1f}m: ${md['btc_dyn_rng']:.0f}")
            diff = md['btc_price'] - md['btc_open']
            dl = self.query_one("#p_btc_diff"); dl.update(f"Diff vs Open: {'+' if diff>=0 else '-'}${abs(diff):.1f}"); dl.classes = "diff_green price_sub" if diff>=0 else "diff_red price_sub"
            tl = self.query_one("#p_btc_trend"); tl.update(f"Trend Str: {md['trend_score']} ({md['trend_prob']:.0%})")
            ol = self.query_one("#p_btc_odds"); ol.update(f"Poly Odds: {md['btc_odds']}/5")
            def set_sig(lid, sig):
            ...
            set_sig("#p_to", md['to_signal']); set_sig("#p_rebound", md['rebound_signal'])
    ...
    def _check_tpsl(self):
        ...
        for bet_id, info in curr_bets:
            ...
            # --- WHALE SPLASH PROTECTION (< 30s) ---
                ...
                rem_sec = 300 - elapsed # Should use 900 for 15m? 
            ...
            if reason:
                ...
                # Sell at Bid - 0.05 for fill
                curr_bid = self.market_data["up_bid"] if side == "UP" else self.market_data["down_bid"]
                limit_price = max(0.02, curr_bid - 0.05)

                if is_live:
                    success, msg = self.live_broker.sell(side, token_id, limit_price=limit_price, best_bid=curr_bid, reason=reason)
                else:
                    success, msg = self.sim_broker.sell(side, curr_bid, reason=reason)
                ...
                if success:
                    ...
                    if self.risk_cap_initialized: self._add_risk_revenue(rev)

    def _process_scanners(self, active_scanners, cb_map):
        # Fetch current config
        try: min_diff = float(self.query_one("#inp_min_diff").value)
        except: min_diff = 25.0
        
        try: min_price = float(self.query_one("#inp_min_price").value)
        except: min_price = 0.20
        
        try: max_price = float(self.query_one("#inp_max_price").value)
        except: max_price = 0.95
        ...
        for name, sig in active_scanners.items():
            ...
            # --- SAFEGUARDS (Skip for Rebound) ---
            skip_safe = (name == "Rebound")

            # --- CHECK: STRONG ONLY ---
            if not skip_safe and self.query_one("#cb_strong").value:
                # Strong Only logic...
            ...
            # Check dupes
            bet_key = f"{name}_{bet_side}"
            if bet_key in self.window_bets: continue
            
            price = self.market_data["up_price"] if bet_side == "UP" else self.market_data["down_price"]
            
            # --- DYNAMIC PRICE FILTER ---
            if not skip_safe and (price < min_price or price > max_price): continue
            
            # --- DYNAMIC DIFF FILTER ---
            if not skip_safe:
                diff = self.market_data["btc_price"] - self.market_data["btc_open"]
                if bet_side == "UP" and diff < min_diff: continue
                if bet_side == "DOWN" and diff > -min_diff: continue
            ...
            # --- DYNAMIC BET SIZING (Trend Based) ---
            max_bet = base_bal * AUTO_BET_PCT # 10% of Bankroll
            trend_prob = self.market_data.get("trend_prob", 0.5)
            
            # Logic: If Trend Strength >= 60%, go Full Size. Else Half Size.
            size_factor = 1.0 if trend_prob >= 0.60 else 0.5
            
            dynamic_amount = max_bet * size_factor
            if dynamic_amount < 1.0: dynamic_amount = 1.0
            
            if self.risk_cap_initialized and dynamic_amount > self.dynamic_risk_cap:
                # Cap at remaining bankroll
                dynamic_amount = self.dynamic_risk_cap
                if dynamic_amount < 1.0: continue
            ...
            if is_live:
                    if dynamic_amount <= self.live_broker.balance and token_id:
                        ...
                        if success:
                            ...
                            self.window_bets[bet_key] = {"entry_price": price, "side": bet_side, "closed": False}
    ...
    def on_risk_update(self, event: Input.Submitted):
        try:
            ...
            self.max_session_cap = new_val # Set Hard Cap
            self.risk_cap_initialized = True
            self.log_msg(f"[bold cyan]Risk Bankroll Manually Updated: ${new_val:.2f} (Max Cap Set)[/]")
            self.update_balance_ui()
        except ValueError:
    ...
    def on_sim_bal_update(self, event: Input.Submitted):
        ...
        try: 
            ...
            if not self.query_one("#cb_live").value:
                self.dynamic_risk_cap = v
                self.max_session_cap = v
    ...
    def _add_risk_revenue(self, amount):
        if not self.risk_cap_initialized: return
        self.dynamic_risk_cap += amount
        # Remove Skimming: Let the bankroll grow with profits
        # if self.dynamic_risk_cap > self.max_session_cap:
        #     self.dynamic_risk_cap = self.max_session_cap
        # self.log_msg(f"[dim green]💰 Added ${amount:.2f} to Risk Cap. New: ${self.dynamic_risk_cap:.2f}[/]")
    ...
    async def trigger_buy(self, side, amount=None):
        ...
        # --- RISK CAP CHECK ---
        if self.risk_cap_initialized and val > self.dynamic_risk_cap + 0.009: 
             self.log_msg(f"[bold white on red]⛔ BLOCKED: Insufficient Risk Cap (${self.dynamic_risk_cap:.2f}). Please update Bankroll.[/]")
             return
        is_live = self.query_one("#cb_live").value
