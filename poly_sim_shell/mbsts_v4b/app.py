...
class SniperApp(App):
    ...
    def __init__(self, sim_broker, live_broker, start_live_mode=False):
        ...
        self.scanning_suspended = False # V4B: Window-level suspension after hedge
        self._market_loop_running = False # Safety lock for overlapping ticks
        self.skipped_scanners = set() # Log suppression tracker

    def compose(self) -> ComposeResult:
        yield Horizontal(
            Label(f"4B-SIM | Bal: ${self.sim_broker.balance:.2f}", id="header_stats"),
    ...
    def update_balance_ui(self):
        ...
        if is_live:
            lbl.update(f"[bold red]4B-LIVE[/] | Bal: ${self.live_broker.balance:.2f}{cap_display}")
            lbl.classes = "live_mode"
        else:
            lbl.update(f"4B-SIM | Bal: ${self.sim_broker.balance:.2f}{cap_display}")
            lbl.classes = ""
    ...
    async def fetch_market_loop(self):
        if self._market_loop_running: return
        self._market_loop_running = True
        try:
            ...
            if not is_first_tick and ts_start != self.market_data["start_ts"]:
                ...
                self.scanning_suspended = False # Reset hedge suppression
                self.window_bets.clear() # EXPLICIT CLEAR for new window
                self.market_data_manager.reset_history() # EXPLICIT CLEAR for new window
                self.skipped_scanners.clear() # Reset log suppression for new window
                for s in self.scanners.values(): s.reset() # EXPLICIT RESET for new window
            ...
            # --- V4B: 1 Trade Max Filter ---
            one_trade_active = self.query_one("#cb_one_trade").value
            window_has_trade = any(not v.get("closed") for v in self.window_bets.values())

            if not self.mid_window_lockout and not getattr(self, "scanning_suspended", False):
                # Optimization: Block all scans if 1 Trade Max is hit
                if one_trade_active and window_has_trade:
                    pass # Continue to price updates, skip scanning logic
                else:
                    for name, sc in self.scanners.items():
                        ...
                        if res and "BET_" in str(res) and not any(k.startswith(f"{name}_") for k in self.window_bets):
                            # Re-verify 1 Trade Max inside loop for intra-tick safety
                            if one_trade_active and any(not v.get("closed") for v in self.window_bets.values()): continue
                            ...
                            if min_diff > 0:
                                ...
                                if diff < min_diff:
                                    if name not in self.skipped_scanners:
                                        self.log_msg(f"[dim]SKIP {name} | Diff ${diff:.2f} < Min ${min_diff:.0f}[/]")
                                        self.skipped_scanners.add(name)
                                    continue
                            ...
                            if "MOM" in str(res): bs = self.portfolios[name].balance * 0.12 # MOM 12% override
                            if bs > 0:
                                ...
                                if pr and 0.01 < pr < 0.99:
                                    ...
                                    # --- V4B HEDGE/CANCEL LOGIC ---
                                    existing_dirs = {v["side"] for v in self.window_bets.values()}
                                    total_exp = sum(v["cost"] for v in self.window_bets.values())
                                    
                                    # Sanity Check: Avoid triggering on insane price data bugs
                                    btc_diff = abs(d["cur"] - d["opn"])
                                    if btc_diff > 1000:
                                        self.log_msg(f"[bold red][SANITY SHIELD]:[/] Blocking {name} signal. Insane BTC Diff: ${btc_diff:,.2f} (Possible API Error)")
                                        continue

                                    if existing_dirs and sd not in existing_dirs:
                                        self.log_msg(f"[bold yellow][HEDGE CONFLICT]:[/] {name} {sd} vs existing {existing_dirs}. Neutralizing ${total_exp:.2f} exposure.")
                                        
                                        # Hedge Safety Check
                                        avail = self.risk_manager.risk_bankroll if not is_l else self.live_broker.balance
                                        if avail < total_exp:
                                            self.log_msg(f"[bold red][CRITICAL HEDGE WARNING]:[/] Insufficient funds (${avail:.2f}) to hedge ${total_exp:.2f}! Exposure remains UNHEDGED.")
                                        
                                        # Execute cancellation buy
                                        ok, msg = await asyncio.to_thread(self.trade_executor.execute_buy, is_l, sd, total_exp, pr, d["poly"]["up_id" if sd=="UP" else "down_id"], context={}, reason=f"HEDGE_{res}")
                                        if ok:
                                            self.log_msg(f"[bold cyan][CANCEL TRADE EXECUTED]:[/] {msg}")
                                            self.scanning_suspended = True # Stop scanning for this window
                                            self.log_msg("[bold red][SCANNING SUSPENDED] for remainder of window.[/]")
                                        else:
                                            self.log_msg(f"[bold red][CANCEL TRADE FAILED]:[/] {msg}")
                                        break # Skip normal buy and stop loop
                                    # ------------------------------
                                    
                                    ok, msg = await asyncio.to_thread(self.trade_executor.execute_buy, is_l, sd, bs, pr, d["poly"]["up_id" if sd=="UP" else "down_id"], context=ctx, reason=res)
                                    if ok:
            ...
            self._market_loop_running = False
    ...
    async def _check_tpsl(self):
        ...
        for bid, info in list(self.window_bets.items()):
            ...
            if reason:
                ...
                ok, msg, revenue = await asyncio.to_thread(self.trade_executor.execute_sell, is_l, side, self.market_data["up_id" if side=="UP" else "down_id"], lp, cbid, reason=reason)
                if ok:
    ...
    def trigger_settlement(self):
        ...
        self.update_balance_ui() # Note: window_bets.clear and scanning_suspended reset moved to next fetch_market_loop tick
    ...
    async def trigger_buy(self, side):
        ...
        # 1 Trade Max Guard
        if self.query_one("#cb_one_trade").value and any(not v.get("closed") for v in self.window_bets.values()):
            return self.log_msg("[bold yellow]1 Trade Max Active. Manual Buy Blocked.[/]")
        ...
        ok, msg = await asyncio.to_thread(self.trade_executor.execute_buy, is_l, side, val, pr or 0.5, tid, context=ctx, reason="Manual")
        if ok:
    ...
    async def trigger_sell_all(self, side):
        ...
        ok, msg, revenue = await asyncio.to_thread(self.trade_executor.execute_sell, is_l, side, tid, lp, cbid, reason="Manual")
        if ok:
            self.log_msg(f"[bold {'red' if is_l else 'green'}]{msg}[/]")
            # Mark ALL relative bets as closed to prevent zombie hedging
            for info in self.window_bets.values():
                if info["side"] == side: info["closed"] = True
            if revenue > 0: self._add_risk_revenue(revenue)
        else:
...
class AlgoInfoModal(ModalScreen):
        ...
        with Vertical(id="modal_container"):
            ...
            if self.algo_id == "MOS":
                ...
                with Horizontal(id="modal_mom_mode"):
                    yield Label("Mode (TIME/PRICE):", id="lbl_mom_mode")
                    yield Input(placeholder="TIME", id="inp_mom_mode")
                with Horizontal(id="modal_mom_threshold"):
                    yield Label("Price Threshold (51-70):", id="lbl_mom_threshold")
    ...
    def on_mount(self):
        ...
        if self.algo_id == "MOS":
            ...
            for row_id in ["#modal_mom_mode", "#modal_mom_threshold"]:
                row = self.query_one(row_id)
                row.styles.height = 3
                row.styles.align = ("center", "middle")
                row.styles.margin = (0, 0, 1, 0)
