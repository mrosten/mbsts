...
class MinOneScanner:
    ...
    def analyze(self, price_history, elapsed):
        ...
        if (h - max(o, c)) > body * 1.5: self.triggered_signal = "BET_DOWN_WICK|Liar's Wick Detected"; return self.triggered_signal
        if (min(o, c) - l) > body * 1.5: self.triggered_signal = "BET_UP_WICK|Liar's Wick Detected"; return self.triggered_signal
        return "WAIT"
...
class AlgoLogger:
    def __init__(self, initial_balance=None):
        # 1. Ask for starting balance
        if initial_balance is None:
            print("\n" + "="*60)
            print("ALGO LOGGER V3 - STARTUP CONFIG")
            print("="*60)
            while True:
                try:
                    raw_bal = input("Enter starting USD balance for EACH algorithm (e.g. 100): ")
                    self.initial_balance = float(raw_bal)
                    if self.initial_balance < 5.50:
                        print("Error: Balance must be >= $5.50 (minimum bet size).")
                        continue
                    break
                except ValueError:
    ...
    def init_logs(self):
        ...
        with open(self.txt_log_path, 'w') as f:
            ...
            f.write(f"Version: V3 (Portfolio Simulation + Dynamic Scaling)\n")
            f.write("="*60 + "\n\n")
    ...
    def run(self):
        ...
        self.log(f"\n{'='*60}")
        self.log(f"ALGO LOGGER v3.0 - PORTFOLIO SIMULATOR")
        self.log(f"{'='*60}")
            ...
            try:
                ...
                if 0 <= elapsed <= 890 and self.live_scanning_active:
                    ...
                    for name, scanner in self.scanners.items():
                        ...
                        elif name == "MidGame": res = scanner.analyze(self.price_history, open_p, elapsed)
