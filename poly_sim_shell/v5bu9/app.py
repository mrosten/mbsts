...
class PulseLeanChart(Static):
    ...
    def save_graph_svg(self, path):
        ...
        try:
            ...
            svg = [f'<svg width="{img_w + pad*2}" height="{img_h + pad*2}" xmlns="http://www.w3.org/2000/svg">']
            # Background
            svg.append(f'<rect width="100%" height="100%" fill="#121212" />')
            ...
            for elapsed, val, side, scanner in self.trigger_events:
                ...
                # The "Title" tag creates the native browser tooltip on hover
                svg.append(f'<circle cx="{x}" cy="{y}" r="4" fill="{color}" stroke="#FFFFFF" stroke-width="1">')
                svg.append(f'  <title>{scanner} {side} ({int(val*100)}¢)</title>')
                svg.append(f'</circle>')
