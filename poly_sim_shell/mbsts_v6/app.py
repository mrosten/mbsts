...
class SniperApp(App):
    ...
    def __init__(self, sim_broker, live_broker, start_live_mode=False):
        ...
        self.session_trade_executed = False
        ...
        self.settings_file = "v6_settings.json"
        ...
        # Base Voter Tier is 2 (Simulated Live Voter)
        self.scanner_tiers = {k: 2 for k in ALGO_INFO.keys()}
        ...
        try:
            if os.path.exists(self.settings_file):
                with open(self.settings_file, "r") as f:
                    saved = json.load(f)
                    if "weights" in saved:
                        for k, v in saved["weights"].items():
                            if k in self.scanner_weights: self.scanner_weights[k] = float(v)
                        for k, v in saved.get("tiers", {}).items():
                            if k in self.scanner_tiers: self.scanner_tiers[k] = int(v)
                    else:
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            Label("Session Bet:", classes="lbl_sm"),
            Input(placeholder="Fixed $", value="50.00", id="inp_fixed_bet_size"),
    ...
    def save_settings(self):
        try:
            with open(self.settings_file, "w") as f:
                json.dump({"weights": self.scanner_weights, "tiers": self.scanner_tiers}, f, indent=4)
    ...
    async def fetch_market_loop(self):
        try:
            ...
            if not is_first_tick and ts_start != self.market_data["start_ts"]:
                ...
                self.session_trade_executed = False # V6 Session Execution Lock
            ...
            if not self.mid_window_lockout and not is_initial_lockout:
                ...
                for name, info in list(self.pending_bets.items()):
                    ...
                    if min_pr <= pr <= max_pr:
                        ...
                        if ok:
                            if name == "Master": self.session_trade_executed = True
                            self.window_bets[f"{name}_{time.time()}"] = {"side":sd,"entry":pr,"cost":bs, "algos": info.get("algos", [])}
                            self.risk_manager.register_bet(bs)
                            if name in self.portfolios:
                                self.portfolios[name].record_trade(sd, pr, bs, bs/pr)
            ...
            # --------------------------------
                ...
                if not self.session_trade_executed:
                    up_votes = 0.0
                    down_votes = 0.0
                    up_algos = []
                    down_algos = []
                    
                    is_live_mode = self.query_one("#cb_live").value

                    for name, sc in self.scanners.items():
                        # The checkbox strictly forces the algorithm completely off (safety bypass)
                        if not self.query_one(scanner_map[name]).value: continue
                        
                        tier = self.scanner_tiers.get(name[:3].upper(), 2)
                        if tier == 0: continue # Tier 0 ignores everything
                        ...
                        if res and "BET_" in str(res):
                            # In Live Mode, ONLY Tier 3 algorithms are allowed to cast valid votes.
                            if is_live_mode and tier < 3: continue
                            # In Sim Mode, Tier 2 and Tier 3 algorithms are allowed to cast valid votes.
                            if not is_live_mode and tier < 2: continue
                            
                            sd = "UP" if "UP" in str(res) else "DOWN"
                            if sd == "UP":
                                up_votes += weight
                                up_algos.append(name[:3].upper())
                            else:
                                down_votes += weight
                                down_algos.append(name[:3].upper())
                                
                    net_diff = up_votes - down_votes
                    if abs(net_diff) > 0:
                        master_side = "UP" if net_diff > 0 else "DOWN"
                        winning_algos = up_algos if master_side == "UP" else down_algos
                        algo_str = "+".join(winning_algos)
                        ...
                        if diff_ok and not any(k.startswith("Master_") for k in self.window_bets):
                            pr = self.market_data["up_ask"] if master_side == "UP" else self.market_data["down_ask"]
                            ...
                            try: fixed_bet = float(self.query_one("#inp_fixed_bet_size").value)
                            except: fixed_bet = 50.0
                            
                            if pr < min_pr or pr > max_pr:
                                if pr < min_pr and "Master" not in self.pending_bets:
                                    self.pending_bets["Master"] = {"side": master_side, "bs": fixed_bet, "res": f"CONSENSUS ({algo_str})", "algos": winning_algos}
                                    self.session_trade_executed = True
                                    try:
                                        lbl = self.query_one(f"#lbl_{winning_algos[0].lower()}")
                                        lbl.add_class("blinking")
                                    except: pass
                                    self.log_msg(f"[dim]MASTER QUEUED {master_side} ({algo_str}) | Waiting for price {min_pr*100:.1f}¢ (Now {pr*100:.1f}¢)[/]")
                            else:
                                if fixed_bet > self.risk_manager.risk_bankroll:
                                    fixed_bet = self.risk_manager.risk_bankroll
                                
                                if fixed_bet > 0.05:
                                    ctx = {
                                        'signal_price': d["cur"], 'rsi': rsi,
                                        'trend': self.market_data_manager.trend_4h,
                                        'risk_bal': self.risk_manager.risk_bankroll
                                    }
                                    ok, msg = self.trade_executor.execute_buy(is_live_mode, master_side, fixed_bet, pr, d["poly"]["up_id" if master_side=="UP" else "down_id"], context=ctx, reason=f"CONSENSUS ({algo_str})")
                                    if ok:
                                        self.session_trade_executed = True
                                        self.window_bets[f"Master_{time.time()}"] = {"side": master_side, "entry": pr, "cost": fixed_bet, "algos": winning_algos}
                                        self.risk_manager.register_bet(fixed_bet)
                                        self.log_msg(f"[bold green]MASTER EXECUTED {master_side} ({algo_str})[/]: {msg}")
                                    else:
                                        self.log_msg(f"[bold red]MASTER FAILED {master_side}[/]: {msg}")
    ...
    def _check_tpsl(self):
        ...
        for bid, info in list(self.window_bets.items()):
            ...
            # 2. UI-configurable TP/SL Check
                ...
                if roi >= tp: reason = f"TP HIT | Ent: {ent*100:.1f}¢ -> {cur*100:.1f}¢ (+{roi*100:.1f}%)"
    ...
    def trigger_settlement(self):
        ...
        for key, info in self.window_bets.items():
            ...
            if info["side"] != winner:
                name = key.split("_")[0]
                if name == "Master":
                    algos = info.get("algos", [])
                    res_str = f"LOSS RECOVERY ({'+'.join(algos)})"
                    self.pending_bets[name] = {"side": info["side"], "bs": info["cost"], "res": res_str, "algos": algos}
                    try:
                        if algos:
                            lbl = self.query_one(f"#lbl_{algos[0].lower()}")
                            lbl.add_class("blinking")
                    except: pass
                    self.log_msg(f"[bold red]RECOVERY QUEUED:[/] Master Trade lost on {info['side']} ({'+'.join(algos)}). Rolled over.")
                else:
...
class AlgoInfoModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="modal_container"):
            ...
            with Horizontal(id="modal_algo_tier"):
                yield Label("Voter Tier (0-3):", id="lbl_algo_tier")
                yield Input(placeholder="2", id="inp_algo_tier")
    ...
    def on_mount(self):
        ...
        row_t = self.query_one("#modal_algo_tier")
        row_t.styles.height = 3
        row_t.styles.align = ("center", "middle")
        row_t.styles.margin = (0, 0, 1, 0)
        row_t.styles.border = ("ascii", "#333333")
        self.query_one("#inp_algo_tier").styles.width = 10
        
        if self.main_app:
            w = self.main_app.scanner_weights.get(self.algo_id, 1.0)
            self.query_one("#inp_algo_weight").value = str(w)
            
            t = self.main_app.scanner_tiers.get(self.algo_id, 2)
            self.query_one("#inp_algo_tier").value = str(t)
    ...
    def close_modal(self):
        if self.main_app:
            try:
                ...
                t = int(self.query_one("#inp_algo_tier").value)
                t = max(0, min(3, t))  # Clamp between 0 and 3
                self.main_app.scanner_tiers[self.algo_id] = t
