...
class AlgoLogger:
    ...
    def init_logs(self):
        ...
        with open(self.csv_signals_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Timestamp", "WindowStart", "Algo", "Signal", "Direction", "Price", "Elapsed"])
    ...
    def log_signal(self, algo_name, signal_text, price, elapsed):
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        # Determine Direction
        direction = "UP" if "UP" in signal_text else "DOWN"
        if "BET_DOWN" in signal_text or "WHALE_LEADER_DOWN" in signal_text or "REVERSAL_DOWN" in signal_text or "SHORT" in signal_text:
            direction = "DOWN"
        elif "BET_UP" in signal_text or "WHALE_LEADER_UP" in signal_text or "REVERSAL_UP" in signal_text or "SNIPER_ENTRY_UP" in signal_text:
            direction = "UP"
            
        # Make signal VERY visible
        print(f"\n\n{'='*70}")
        ...
        self.log(f"[{timestamp}] SIGNAL: {algo_name} -> {direction} @ ${price:,.2f} (T+{elapsed}s)")
        
        with open(self.csv_signals_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([timestamp, self.current_window_start, algo_name, signal_text, direction, price, elapsed])
    ...
    def settlement_summary(self, close_price, open_price):
        ...
        if not self.active_signals:
            ...
            self.log(f"{'ALGO':<20} | {'DIR':<5} | {'RESULT':<8}")
            self.log("-" * 50)
            ...
            for sig in self.active_signals:
                ...
                result = "LOSS"
                # Win if the window closed in the predicted direction (Green vs Red)
                # Entry price is irrelevant for the win condition, only direction matters.
                if sig["direction"] == "UP" and close_price > open_price: result = "WIN"
                elif sig["direction"] == "DOWN" and close_price < open_price: result = "WIN"
                elif close_price == open_price: result = "DRAW"
                
                if result == "WIN": wins += 1
                
                self.log(f"{sig['algo']:<20} | {sig['direction']:<5} | {result:<8}")
    ...
    def print_final_summary(self):
        ...
        valid_total = sum(s['signals'] - s['pending'] for s in algo_stats.values())
        overall_win_rate = (total_wins / valid_total * 100) if valid_total > 0 else 0
        self.log("-"*60)
        
        total_res = f"{total_wins}/{total_signals - total_wins - sum(s['pending'] for s in algo_stats.values())}/{sum(s['pending'] for s in algo_stats.values())}"
    ...
    def print_status_line(self, price, elapsed, up_p, down_p, open_price):
        ...
        # Using \r to overwrite line. 
        # Added Open and Diff
        print(f"\r[T-{mins}:{secs:02d}] BTC:{price:,.2f} (Op:{open_price:,.2f} D:{sign}{diff:.2f}) | {leader}:${l_price:.2f} (U:{up_p:.2f} D:{down_p:.2f}) | Sigs:{signal_count}   ", end="", flush=True)
            ...
            try:
                ...
                if window_start != self.current_window_start:
                    ...
                    if self.current_window_start > 0:
                        # Logic to handle window close
                        # We might have missing data if we just started, but let's close what we have
                        if self.price_history:
                            close_price = self.price_history[-1]['price']
                            open_price = self.price_history[0]['price']
                            # self.settle_window(close_price) # Removed as it does not exist
                    ...
                    self.log(f"[{now.strftime('%H:%M:%S')}] NEW WINDOW STARTED: {window_start}")
                ...
                for p_sig in self.pending_signals[:]:
                    ...
                    if crossed:
                        self.log_signal(p_sig['algo'], p_sig['signal'], price, elapsed)
                        self.pending_signals.remove(p_sig)
                         ...
                         if scanner.triggered_signal == res: 
                             ...
                             if not already_logged and not already_pending:
                                 ...
                                 if is_winning:
                                     ...
                                     # Queue it
                                     print(f"⏳ SIGNAL QUEUED: {name} wants {direction} (Price ${price:,.2f} vs Open ${open_price:,.2f})")
