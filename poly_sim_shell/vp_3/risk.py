...
class RiskManager:
    ...
    """
    def __init__(self, config: TradingConfig, is_live_mode=False):
        self.config = config
        ...
        self.bet_percentage = self.config.DEFAULT_RISK_PCT  # Default 12% of risk bankroll per bet
    def set_bankroll(self, amount, is_live=False):
        self.is_live_mode = is_live
        if is_live:
            self.risk_bankroll = amount / self.config.LIVE_RISK_DIVISOR
        else:
...
class AlgorithmPortfolio:
    def __init__(self, algo_name, initial_balance, config: TradingConfig):
    ...
    def calculate_bet_size(self, context, strategy_type):
        ...
        cost = base_capital * self.config.DEFAULT_RISK_PCT
    ...
    def settle_window(self, win_side, close_price, open_price):
        ...
        if self.balance < self.config.MIN_BET:
