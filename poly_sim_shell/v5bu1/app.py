...
class SniperApp(App):
        ...
        for bid, info in list(self.window_bets.items()):
            ...
            if reason:
                ...
                if ok:
                    ...
                    # --- SL+ RECOVERY LOGIC ---
                    if reason.startswith("SL HIT") and self.sl_plus_mode and realized_loss > 0:
                        opp_side = "DOWN" if side == "UP" else "UP"
                        opp_p = self.market_data["up_ask" if opp_side == "UP" else "down_ask"]
                        if 0.05 < opp_p < 0.90:
                            # Formula: Cost = Loss * P / (1-P)
                            rec_cost = realized_loss * opp_p / (1 - opp_p)
                            cap = self.risk_manager.risk_bankroll * 0.12
                            if rec_cost > cap:
                                self.log_msg(f"[yellow]SL+ RECOVERY CAP:[/][dim] Calculated ${rec_cost:.2f} but capped at 12% Bankroll (${cap:.2f})[/]")
                                rec_cost = cap
    ...
    async def trigger_sell_all(self, side):
        ...
        ok, msg = self.trade_executor.execute_sell(is_l, side, tid, lp, cbid, reason="Manual")
        if ok:
            self.log_msg(f"[bold {'red' if is_l else 'green'}]{msg}[/]")
            try: 
