...
class TradeEngineMixin:
    ...
    async def fetch_market_loop(self):
        ...
        try:
            ...
            if is_new_window:
                ...
                self.log_msg(f"WIN #{ts_start} | Open: [bold white]${opn:,.2f}[/]{active_str}", level="ADMIN")
            ...
            scanner_map = {"NPattern":"#cb_npa","Fakeout":"#cb_fak","Momentum":"#cb_mom","RSI":"#cb_rsi","TrapCandle":"#cb_tra","MidGame":"#cb_mid","LateReversal":"#cb_lat","BullFlag":"#cb_sta","CoiledCobra":"#cb_cco","PostPump":"#cb_pos","StepClimber":"#cb_ste","Slingshot":"#cb_sli","MinOne":"#cb_min","Liquidity":"#cb_liq","Cobra":"#cb_cob","Mesa":"#cb_mes","MeanReversion":"#cb_mea","GrindSnap":"#cb_gri","VolCheck":"#cb_vol","Moshe":"#cb_mos","ZScore":"#cb_zsc"}
                ...
                for name, info in list(self.pending_bets.items()):
                    ...
                    if min_pr <= pr <= max_pr:
                        ...
                        if ok:
                            ...
                            self.log_msg(f"[bold green]EXEC {name}[/]: {msg} ({short_res})")
                        else:
            ...
            # --------------------------------
                ...
                for name, sc in self.scanners.items():
                    ...
                    if res == "WAIT" and name == "NPattern": res = sc.analyze(ph, d["opn"])
                    ...
                    elif res == "WAIT" and name == "Momentum" and self.mom_buy_mode != "PRE":
                        ...
                        # Calculate current odds score (sentiment) for the scanner
                        cur_odds = round((d["poly"]["up_bid"] - d["poly"]["down_bid"]) * 100, 1)
                        
                        # Pass dynamic filtering settings from app state
                        res = sc.analyze(
                            elapsed, 
                            d["poly"]["up_bid"], d["poly"]["down_bid"], 
                            atr_5m=self.market_data_manager.atr_5m, 
                            trend_4h=self.market_data_manager.trend_4h,
                            atr_floor=self.mom_adv_settings.get("atr_floor", 25.0),
                            trend_penalty_val=self.mom_adv_settings.get("trend_penalty", 0.02),
                            decisive_diff_val=self.mom_adv_settings.get("decisive_diff", 0.02),
                            odds_score=cur_odds
                        )
                    ...
                    elif res == "WAIT" and name == "BullFlag": res = sc.analyze(d["c60"], atr_5m=self.market_data_manager.atr_5m)
                    elif res == "WAIT" and name == "CoiledCobra": res = sc.analyze(d["c60"], d["cur"], elapsed)
                    ...
                    if "BET_" in str(res) and not any(k.startswith(f"{name}_") for k in self.window_bets):
                        ...
                        try:
                            ...
                            if is_pct:
                                # Percentage of CURRENT Risk Bankroll
                                bs = self.risk_manager.risk_bankroll * (ui_amount / 100.0)
                            else:
                                # Fixed Dollar Amount
                                bs = ui_amount
                        except:
                            # Default fallback if UI read fails
                            bs = self.risk_manager.risk_bankroll * 0.10 if self.risk_manager.risk_bankroll > 0 else 1.00
                        ...
                        if bs > 0:
                            ...
                            if pr and 0.01 < pr < 0.99:
                                ...
                                if ok:
                                    ...
                                    short_res = str(res).replace("BET_UP_", "").replace("BET_DOWN_", "").replace("Leader @ 10s:", "Ldr:").replace(" UP ", " ").replace(" DOWN ", " ")
                                    self.log_msg(f"{msg} ({short_res})", level="TRADE")
                                else:
            ...
            def _fetch_next():
                try:
                    ...
                    if up_bid > 0 or dn_bid > 0:
                        fmt_msg = (
                            f"NEXT WIN: "
                            f"UP {up_bid*100:.1f}/{up_ask*100:.1f}¢ | "
                            f"DN {dn_bid*100:.1f}/{dn_ask*100:.1f}¢"
                        ).replace(".0/", "/").replace(".0¢", "¢")
                        self.call_from_thread(self.log_msg, fmt_msg)
                    else:
        ...
        if (rem <= 15 and not self.pre_buy_triggered
            ...
            def _do_prebuy():
                try:
                    ...
                    if self.mom_buy_mode == "ADV" or self.mom_buy_mode == "PRE":
                        atr = getattr(self.market_data_manager, "atr_5m", 0)
                        adv = self.mom_adv_settings
                        atr_floor = adv.get("atr_floor", 25.0)
                        
                        if atr < atr_floor:
                            self.call_from_thread(self.log_msg, f"[dim]PRE-BUY skipped (ATR {atr:.1f} < Floor {atr_floor:.1f})[/]")
                            return

                        if atr >= adv.get("atr_high", 40) and adv.get("auto_stn_chaos", False):
                            self.call_from_thread(self.log_msg, f"[dim]PRE-BUY chaos skip (ATR:{atr:.1f}) -> Forcing STN[/]")
                            return

                    # Calculate current odds score (sentiment)
                    up_b = self.market_data.get("up_bid", 0)
                    dn_b = self.market_data.get("down_bid", 0)
                    cur_odds = round((up_b - dn_b) * 100, 1)
                    ...
                    if side and price:
                        # Sentiment Guard: Skip if sentiment strongly opposes decision
                        if side == "UP" and cur_odds <= -40:
                            self.call_from_thread(self.log_msg, f"[dim]PRE-BUY UP skipped (Sentiment Guard: {cur_odds})[/]")
                            return
                        if side == "DOWN" and cur_odds >= 40:
                            self.call_from_thread(self.log_msg, f"[dim]PRE-BUY DOWN skipped (Sentiment Guard: {cur_odds})[/]")
                            return
                        ...
                        if ok:
                            pending = {"side": side, "entry": price, "cost": bet_size, "algorithm": "Momentum"}
    ...
    def trigger_settlement(self):
        ...
        self.log_msg(f"PnL: [bold {color}]{'+' if net_pnl >= 0 else ''}${net_pnl:.2f}[/] | Revenue: [bold gold3]${total_window_revenue:.2f}[/]", level="MONEY")
        ...
        # 5. Bankroll Refill Logic
        try:
            # Refill working bankroll from total balance up to the UI target
            target_br = float(self.query_one("#inp_risk_alloc").value)
            current_bal = self.live_broker.balance if self.query_one("#cb_live").value else self.sim_broker.balance
            
            # The working bankroll fills up to target, capped by total balance
            self.risk_manager.risk_bankroll = min(current_bal, target_br)
            self.log_msg(f"Bankroll Refilled: [bold gold3]${self.risk_manager.risk_bankroll:.2f}[/] (Target: ${target_br:.2f})", level="MONEY")
        except: pass
