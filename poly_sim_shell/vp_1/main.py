...
def main():
    ...
    default_log = os.path.join(log_dir, f"pulse_log_5M_{datetime.now().strftime('%Y%m%d_%H%M')}.csv")
    ...
    app = PulseApp(sim_broker, live_broker, start_live_mode=is_live_start)
    app.run()
