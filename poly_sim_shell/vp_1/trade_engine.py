...
class TradeEngineMixin:
    ...
    async def fetch_market_loop(self):
        ...
        try:
            ...
            if is_new_window:
                ...
                # Clear strike for new window (will be populated 3s later)
            ...
            if is_new_window:
                # 0. Trigger Settlement for the PREVIOUS window before updating start_ts
                self.trigger_settlement()
                self.audit_completed = False # Reset audit latch for new window
                
                # 1. Capture current window strike (will be compared at T=30)
            ...
            # 3. Background Audit at T >= 30
            if elapsed >= 30 and not getattr(self, "audit_completed", False) and getattr(self, "audit_buffer", None):
                self.audit_completed = True # Latch immediately to prevent double-fire
                buf = self.audit_buffer
                self.audit_buffer = None 
                
                next_strike = poly.get("p2b") if poly else None
                if next_strike:
                    p2b_prev = buf.get("strike_o")
                    p2b_next = next_strike
                    
                    actual_winner = None
                    if p2b_prev and p2b_next:
                        if float(p2b_next) > float(p2b_prev): actual_winner = "UP"
                        elif float(p2b_next) < float(p2b_prev): actual_winner = "DOWN"
                    
                    if actual_winner:
                        if actual_winner == buf["winner_optimistic"]:
                            self.log_msg(f"[30s AUDIT]: Result Confirmed ✅", level="SYS")
                        else:
                            # DISCREPANCY DETECTED!
                            self.log_msg(f"[bold red]⚠️ AUDIT DISCREPANCY![/] Local={buf['winner_optimistic']} | Actual={actual_winner}", level="ERROR")
                            
                            # Apply corrections for SIM mode
                            if not self.query_one("#cb_live").value:
                                # 1. Calculation: What we got vs what we should have gotten
                                # Initial payout was based on winner_optimistic
                                initial_payout = buf["shares"].get(buf["winner_optimistic"], 0)
                                actual_payout = buf["shares"].get(actual_winner, 0)
                                
                                adjustment = actual_payout - initial_payout
                                self.log_msg(f"[bold yellow]AUDIT CORRECTION:[/] Adjusting Eq/PnL by {adjustment:+.2f}...", level="SYS")
                                
                                # 2. Apply to financials
                                self.sim_broker.balance += adjustment
                                self.session_pnl += adjustment
                                self.update_balance_ui()
                                
                                # 3. Log the final corrected state for auditing purposes
                                self.log_msg(f"CORRECTED: PnL Adjusted to {self.session_pnl:+.2f} | Eq: ${self.sim_broker.balance:.2f}", level="STATS")
                else:
                    self.log_msg(f"[30s AUDIT]: Skipping (Poly API Strike Unavailable)", level="SYS")
    ...
    async def _run_last_second_exit(self, ui_is_live):
        ...
        for side, is_live in open_bets_grouped:
            ...
            if not is_live:
                # SIM MODE: Perform "True Selling" at the current bid
                ...
                if shares > 0:
                    revenue = shares * cbid
                    self.sim_broker.balance += revenue
                    self.risk_manager.register_revenue(revenue)
                    
                    action = "SELL UP" if side == "UP" else "SELL DOWN"
                    msg = f"{action} | Sz: {shares:.2f} | Ext: {cbid*100:.1f}c | [bold green]${revenue:.2f}[/] | [SIM Exit]"
                    self.log_msg(msg, level="RSLT")
                    
                    # Mark all bets on this side as closed
                    for info in self.window_bets.values():
    ...
    async def update_timer(self):
        ...
        if rem <= 5 and not self.last_second_exit_triggered:
    ...
    def trigger_settlement(self):
        ...
        # v5.9.7: Simplified investment tracking. 
        # Skip trades that were already closed by Safety Exit or TP/SL
        active_bets = [info for info in self.window_bets.values() if not info.get("closed")]
        window_invested = sum(info.get("cost", 0) for info in active_bets)
        ...
        # 4. Store for Audit at T=30
        self.audit_buffer = {
            "window_id": self.market_data.get("start_ts", 0),
            "winner_optimistic": winner,
            "strike_o": self.market_data.get("window_strike"),
            "pnl": net_pnl,
            "shares": {
