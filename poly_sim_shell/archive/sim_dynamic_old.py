"""
Similar to sim_trend_t9.py - also, non correct open price.


Simulator: (Deprecated) Paper Trader

A 'Paper Trading' bot that runs in real-time but uses fake money.
- Connects to live API for data.
- Simulates execution (does not sign transactions).
...
class CLISimulation:
    def __init__(self, balance, resume=False):
        ...
        self.trade_size = 5.5 # Deprecated, used as base fallback? No, we calculate dynamically.
    ...
    # --- Main Loop ---
    async def run(self):
        print(f"--- PolySim DYNAMIC CLI (Event Based) ---")
        print(f"Initial Balance: ${self.balance:.2f} | Strategy: Dynamic Sizing (5% - 33%)")
        ...
        while self.is_running:
            try:
                ...
                # Strategy Execution Logic (Moved BEFORE Plan)
                drift_pct = 0.0
                ...
                if (check_t6 or check_t9) and not self.strategy_triggered and not self.active_trade:
                    if drift_pct > 0.0004:
                         ...
                         if price > 0:
                             # SAFETY: Prevent Contra-Trend Buys (Matched to Live Bot)
                             if price < 0.55:
                                 # self.log(f"SKIPPING: {side} @ {price:.2f} too low/risky. (< $0.55)")
                                 self.strategy_triggered = True
                                 continue

                             # Dynamic Sizing Logic
                             # Base 5% + (Price - 0.5) * 0.56
                             wager_pct = 0.05 + max(0, (price - 0.5)) * 0.56
                             ...
                             self.log(f"    -> Wager {wager_pct:.1%} of Bal (${cost:.2f}) -> {shares:.2f} Shares")
                             self.save_state()
                    else:
