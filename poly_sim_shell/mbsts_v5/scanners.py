...
class NPatternScanner(BaseScanner):
    def __init__(self, min_impulse_size=0.0003, max_retrace_depth=0.85):
        super().__init__()
...
class FakeoutScanner(BaseScanner):
    def __init__(self, phase1_duration=60):
        super().__init__()
...
class MomentumScanner(BaseScanner):
    ...
    def get_signal(self, context: dict) -> str:
        ...
        # -------------------------------------------------------------
        if elapsed >= 285 and not self.pre_buy_triggered and self.buy_mode in ["PRE", "HYBRID", "ADV"]:
            # ADV Logic Override
...
class Momentum2Scanner(BaseScanner):
    ...
    def get_signal(self, context: dict) -> str:
        ...
        # 🟢 Filter 1: Static Gates (ATR & Timing)
        if atr_5m is not None and atr_5m < atr_floor:
            self.last_skip_reason = f"Low Vol (<${atr_floor})"
            return "NONE"
...
class RsiScanner(BaseScanner):
    def __init__(self, rsi_threshold=20, time_remaining_min=80):
        super().__init__()
    ...
    def get_signal(self, context: dict):
        ...
        time_remaining = 300 - context.get("elapsed", 0)
        
        if rsi < self.rsi_threshold and price < bb_lower and time_remaining > self.time_remaining_min:
            self.triggered_signal = f"BET_UP_RSI_OVERSOLD|RSI {rsi:.1f} + Below BB"
            return self.triggered_signal
        return "WAIT"

class TrapCandleScanner(BaseScanner):
    def __init__(self, start_move_pct=0.002, retrace_ratio=0.35):
        super().__init__()
...
class MidGameScanner(BaseScanner):
    def __init__(self, min_elapsed=90, max_green_ticks=25):
        super().__init__()
    ...
    def get_signal(self, context: dict):
        ...
        if elapsed < self.min_elapsed: return "WAIT_TIME"
        crossed_up = any(x['price'] > open_price for x in price_history if self.min_elapsed <= x['elapsed'] <= 200)
        green_ticks = sum(1 for x in price_history if x['price'] > open_price and self.min_elapsed <= x['elapsed'] <= 200)
        if crossed_up and green_ticks < self.max_green_ticks and price_history[-1]['price'] < open_price and elapsed > 200:
             self.triggered_signal = "BET_DOWN_FAILED_RESCUE|Bulls failed to hold green"
             return self.triggered_signal
        return "WAIT"

class LateReversalScanner(BaseScanner):
    def __init__(self, min_elapsed=200, surge_pct=1.0003):
        super().__init__()
        self.min_elapsed = min_elapsed
...
class PostPumpScanner(BaseScanner):
    def __init__(self, min_pump_pct=0.003, reversal_depth=0.9985):
        super().__init__()
...
class StepClimberScanner(BaseScanner):
    def __init__(self, tick_count=15, climb_pct=1.001):
        super().__init__()
...
class SlingshotScanner(BaseScanner):
    def __init__(self, ma_period=15):
        super().__init__()
...
class MinOneScanner(BaseScanner):
    def __init__(self, wick_multiplier=1.5):
        super().__init__()
...
class LiquidityVacuumScanner(BaseScanner):
    def __init__(self, break_structure_pct=1.0001):
        super().__init__()
        self.swept = False
        self.sweep_high = 0
...
class CobraScanner(BaseScanner):
    def __init__(self, ma_period=15, std_dev_mult=1.5):
        super().__init__()
        self.ma_period = ma_period
    ...
    def get_signal(self, context: dict):
        ...
        current_price = context.get("current_price", 0.5)
        elapsed = context.get("elapsed", 0)
        if elapsed > 180 or len(closes_60m) < self.ma_period: return "WAIT"
...
class MesaCollapseScanner(BaseScanner):
    def __init__(self, pump_threshold=0.001, cross_count=2):
        super().__init__()
    ...
    def get_signal(self, context: dict):
        ...
        if self.state == "SEARCHING":
            if elapsed <= 60 and (current_price - open_price) / open_price > self.pump_threshold:
...
class MeanReversionScanner(BaseScanner):
    def __init__(self, reversal_threshold=0.0005):
        super().__init__()
...
class GrindSnapScanner(BaseScanner):
    def __init__(self, grind_duration=100, snap_duration=20, min_slope_pct=0.1, reversal_ratio=0.60):
        super().__init__()
    ...
    def get_signal(self, context: dict):
        ...
        total_required_time = self.grind_duration + self.snap_duration
        ...
        p_snap_start = next((x['price'] for x in reversed(price_history) if x['elapsed'] <= (elapsed - 10 - self.snap_duration)), None)
        p_grind_start = next((x['price'] for x in reversed(price_history) if x['elapsed'] <= (elapsed - 10 - self.snap_duration - self.grind_duration)), None)
        ...
        recent_snap_period = self.get_price_slice(price_history, elapsed - 10 - self.snap_duration, elapsed)
...
class VolCheckScanner(BaseScanner):
    def __init__(self, avg_3m_multiplier=1.0):
        super().__init__()
    ...
    def get_signal(self, context: dict):
        ...
        open_price = context.get("open_price", 0.5)
        elapsed = context.get("elapsed", 0)
        up_p = context.get("up_ask", 0.5) # Assuming we want asks for pricing threshold
...
class MosheSpecializedScanner(BaseScanner):
    def __init__(self, t1=290, d1=2000.0, t2=80, d2=80.0, t3=15, d3=25.0, moshe_threshold=0.86):
        super().__init__()
        self.checkpoints = {}
...
class ZScoreBreakoutScanner(BaseScanner):
    def __init__(self, z_threshold=3.5, coil_threshold=0.001):
        super().__init__()
    ...
    def get_signal(self, context: dict):
        ...
        thresh = self.z_threshold if elapsed < 260 else (self.z_threshold - 0.5)
        if abs(z) > thresh:
...
class NitroScanner(BaseScanner):
    ...
    def __init__(self):
        super().__init__()
        self.time_cutoff = 120 # First 2 minutes default
...
class VolSnapScanner(BaseScanner):
    ...
    def __init__(self):
        super().__init__()
        self.time_cutoff = 60 # Default: first 60 seconds
    ...
    def get_signal(self, context: dict) -> str:
        ...
        if elapsed <= self.time_cutoff:
             ...
             self.last_skip_reason = f"ACCUMULATING: U{self.cum_up:.0f} D{self.cum_dn:.0f} (T-{self.time_cutoff-elapsed}s)"
             return "WAIT"
...
ALGO_INFO = {
    ...
    "MOS": {"name": "Moshe Sniper", "desc": "Surge detection based on 30-second price checkpoints and 1H macro trend alignment."},
