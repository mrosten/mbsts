...
from textual.widgets import Header, Footer, Input, Button, RichLog, Label, Checkbox, Static
...
class SniperApp(App):
    ...
    def __init__(self, sim_broker, live_broker, start_live_mode=False):
        ...
        self.ghost_logged = set()
        self.only_pending_mode = True  # V5B: Ghost mode for fresh signals (DEFAULT ON)
        ...
        self.settings_file = "v5b_settings.json"
    ...
    def on_input_submitted(self, event: Input.Submitted):
        ...
        if event.input.id == "inp_cmd":
            ...
            elif cmd == "onlypending=true":
                self.only_pending_mode = True
                self.log_msg("[bold yellow]V5B: Only-Pending Mode ENABLED.[/] Fresh signals are ghost trades. Pending/recovery bets execute for real.")
                self.update_balance_ui()
            elif cmd == "onlypending=false":
                self.only_pending_mode = False
                self.log_msg("[bold green]V5B: Only-Pending Mode DISABLED.[/] All signals execute normally.")
                self.update_balance_ui()
            elif cmd == "?":
    ...
    def save_settings(self):
        try:
            with open(self.settings_file, "w") as f:
                json.dump(self.scanner_weights, f, indent=4)
    ...
    def update_balance_ui(self):
        ...
        if is_live:
            ...
            op_tag = " [bold yellow][OP][/]" if self.only_pending_mode else ""
            lbl.update(f"5B-SIM{op_tag} | Bal: ${self.sim_broker.balance:.2f}{cap_display}")
            lbl.classes = ""
    ...
    async def fetch_market_loop(self):
        try:
            ...
            if not is_first_tick and ts_start != self.market_data["start_ts"]:
                ...
                self.ghost_logged.clear() # reset ghost log tracker
                
                # --- V5B ONLY-PENDING: Priority + Direction Filter on new window ---
                if self.only_pending_mode and self.pending_bets:
                    PRIORITY = {"LOSS_RECOVERY": 0, "MIN_QUEUE": 1, "MAX_QUEUE": 2}
                    best = min(PRIORITY.get(v.get("source", "MAX_QUEUE"), 2) for v in self.pending_bets.values())
                    top = {k: v for k, v in self.pending_bets.items() if PRIORITY.get(v.get("source", "MAX_QUEUE"), 2) == best}
                    up_n = sum(1 for v in top.values() if v["side"] == "UP")
                    dn_n = sum(1 for v in top.values() if v["side"] == "DOWN")
                    win_dir = "UP" if up_n >= dn_n else "DOWN"
                    keep = {k: v for k, v in top.items() if v["side"] == win_dir}
                    cancel = [k for k in self.pending_bets if k not in keep]
                    for k in cancel:
                        try: self.query_one(f"#lbl_{k[:3].lower()}").remove_class("blinking")
                        except: pass
                    if cancel:
                        self.log_msg(f"[dim red][OP] Dropped {len(cancel)} lower-priority/off-dir pending: {', '.join(cancel)}[/]")
                    self.pending_bets = keep
                # ---------------------------------------------------------------
            ...
            # Mid-Window Safety Lockout Activation (DISABLED by default in V5B per user request lo=false)
            ...
            # --------------------------------
                ...
                for name, sc in self.scanners.items():
                    ...
                    if res and "BET_" in str(res) and not any(k.startswith(f"{name}_") for k in self.window_bets):
                        ...
                        # --- V5B ONLY-PENDING MODE GATE (must be here, before any queuing) ---
                        if self.only_pending_mode:
                            if name not in self.ghost_logged:
                                self.log_msg(f"[dim cyan][GHOST] {name} {'UP' if 'UP' in str(res) else 'DOWN'} | {str(res)[:40]}[/]")
                                self.ghost_logged.add(name)
                            continue
                        # -------------------------------------------------------------------
                        ...
                        if bs > 0:
                            ...
                            if pr < min_pr or pr > max_pr:
                                if pr < min_pr and name not in self.pending_bets:
                                    ...
                                    self.pending_bets[name] = {"side": sd, "bs": bs, "res": res, "source": "MIN_QUEUE"}
                                    try:
                                        lbl = self.query_one(f"#lbl_{name[:3].lower()}")
    ...
    def trigger_settlement(self):
        ...
        for key, info in self.window_bets.items():
            ...
            if info["side"] != winner:
                name = key.split("_")[0]
                self.pending_bets[name] = {"side": info["side"], "bs": info["cost"], "res": f"LOSS RECOVERY ({name})", "source": "LOSS_RECOVERY"}
    ...
    def _add_risk_revenue(self, amount):
        ...
        # 2. Hard Clamping: Never exceed the Target Bankroll (theoretical risk bankroll)
        ...
        # 3. Always cap by wallet balance as well
...
class HelpModal(ModalScreen):
    """A modal that shows available commands."""
    def compose(self) -> ComposeResult:
        with Vertical(id="modal_container"):
            yield Static("[bold #00ffff]V5B Terminal Commands[/]", id="modal_title")
            yield Static(
                "• [bold cyan]onlypending=true[/] : Ghost fresh signals, only execute carry-overs\n"
                "• [bold cyan]onlypending=false[/]: Normal trading mode\n"
                "• [bold cyan]lo=false[/]         : Cancel mid-window safety lockout\n"
                "• [bold cyan]?[/]                : Show this help menu",
                id="help_text"
            )
            with Horizontal(id="modal_footer"):
                yield Button("CLOSE", id="btn_modal_close", variant="primary")
    ...
    def on_mount(self):
        ...
        self.query_one("#help_text").styles.margin = (1, 0)
        self.query_one("#help_text").styles.padding = (0, 2)
