...
class StatTracker:
    ...
    def record_result(self, is_win, is_draw):
        if is_win:
            self.total_wins += 1
            self.consecutive_losses = 0
        elif is_draw:
    ...
    def calculate_bet_size(self, balance, context, strategy_type):
        """
        Scaling logic inspired by sim_live_trend_linear_v4_2.py
        Calculates bet size based on GLOBAL balance.
        """
        if balance < 5.50: return 0
        ...
        if cost < MIN_BET:
            # If we have enough for Min, use Min. Otherwise 0.
            cost = MIN_BET if balance >= MIN_BET else 0
...
class AlgoLogger:
    def __init__(self, initial_balance=None):
        ...
        if initial_balance is None:
            ...
            while True:
                try:
                    raw_bal = input("Enter starting USD balance (GLOBAL wallet): ")
        ...
        # Initialize Global Wallet & Stats
        self.global_balance = self.initial_balance
        self.stats = {name: StatTracker(name) for name in self.scanners}
        
        self.price_history = []
        self.active_trades = [] # List of {algo, side, entry, cost, shares, timestamp} for CURRENT window
    ...
    def init_logs(self):
        ...
        with open(self.txt_log_path, 'w') as f:
            ...
            f.write(f"Initial Global Balance: ${self.initial_balance:.2f}\n")
            f.write(f"Version: V4 CASCADE (Priority Queue)\n")
    ...
    def log_signal(self, algo_name, signal_text, price, elapsed, is_pending=False, poly_price=0.50):
        ...
        if not is_pending and tracker:
            ...
            if bet_size > 0:
                ...
                print(f"\n\n{'='*70}")
                print(f"🚨 CASCADE TRADE EXECUTED: {algo_name} → {direction}")
                ...
                print(f"Global Equity: ${self.global_balance:.2f} remaining")
                print(f"{'='*70}\n")
                
                self.log(f"[{timestamp}] TRADE: {algo_name} -> {direction} | Bet: ${bet_size:.2f} @ ${poly_price:.2f} | Global Bal: ${self.global_balance:.2f} (T+{elapsed}s)")
            else:
                self.log(f"[{timestamp}] SKIP: {algo_name} insufficient global balance or scaling logic returned 0.")
        elif is_pending:
        ...
        try:
            with open(self.csv_signals_path, 'a', newline='') as f:
                ...
                writer.writerow([timestamp, self.current_window_start, algo_name, event_type, direction, price, bet_size, self.global_balance, signal_text])
        except: pass
    ...
    def settlement_summary(self, close_price, open_price):
        ...
        if not self.active_trades:
            ...
            self.log(f"{'ALGO':<15} | {'DIR':<4} | {'STATUS':<10} | {'COST':<8} | {'PNL':<8} | {'GLOBAL BAL'}")
            self.log("-" * 80)
            
            for trade in self.active_trades:
                if trade["window_start"] != self.current_window_start: continue
                
                algo_name = trade['algo']
                tracker = self.stats.get(algo_name)
                ...
                self.global_balance += payout
                profit = payout - trade['cost']
                
                # Show Price Paid (Contract Cost) in settlement log if available
                contract_cost = f"${trade['poly_price']:.2f}"
                self.log(f"{algo_name:<15} | {trade['side']:<4} | {status:<10} | ${trade['cost']:<7.2f} (@{contract_cost}) | ${profit:<7.2f} | ${self.global_balance:.2f}")
                ...
                try:
                    with open(self.csv_results_path, 'a', newline='') as f:
                        writer = csv.writer(f)
                        writer.writerow([
                            trade["window_start"], algo_name, trade["side"], 
                            trade["entry_price"], close_price, status, trade["cost"], payout, profit, self.global_balance
                        ])
                except: pass
    ...
    def print_final_summary(self):
        ...
        self.log("="*80)
        self.log(f"{'ALGORITHM':<15} | {'W/L/D':<10} | {'NET CONTRIBUTION'}")
        self.log("-" * 80)
        
        # Sort by Wins descending
        sorted_stats = sorted(self.stats.values(), key=lambda x: x.total_wins, reverse=True)
        
        for p in sorted_stats:
            res_str = f"{p.total_wins}/{p.total_losses}/{p.total_draws}"
            self.log(f"{p.algo_name:<15} | {res_str:<10} | (Shared Wallet)")
            
        self.log("-" * 80)
        total_pnl = self.global_balance - self.initial_balance
        self.log(f"{'GLOBAL WALLET':<15} | {'':<10} | FINAL: ${self.global_balance:.2f} (PnL: ${total_pnl:+.2f})")
        self.log("="*80 + "\n")
    ...
    def print_status_line(self, price, elapsed, up_p, down_p, open_price):
        ...
        # Portfolio aggregate
        total_bal = self.global_balance
        
        line = f"[T-{mins}:{secs:02d}] BTC:{price:,.2f} {arrow} | {leader}:${l_price:.2f} (U:{up_p:.2f} D:{down_p:.2f}) | Global:${total_bal:,.0f}"
    ...
    def run(self):
        ...
        self.log(f"\n{'='*60}")
        self.log(f"ALGO LOGGER v4.0 - CASCADE MODE")
        self.log(f"Priority: Slingshot > Cobra > GrindSnap > Others")
        if self.test_mode: self.log("!!! RUNNING IN TEST MODE (SYNTHETIC DATA) !!!")
        self.log(f"{'='*60}")
        self.log(f"Active Scanners: {len(self.scanners)}")
        self.log(f"Global Balance: ${self.initial_balance:.2f}")
        self.log("-" * 60)
        ...
        while True:
            try:
                ...
                if 0 <= elapsed <= 890 and self.live_scanning_active:
                    ...
                    # --- CASCADE SCANNER LOGIC ---
                    tick_signals = [] # Collect all valid signals for this tick
                    ...
                    for name, scanner in self.scanners.items():
                        ...
                        try:
                            ...
                            if res and "BET_" in str(res):
                                tick_signals.append({'algo': name, 'res': res})
                        except Exception as e:
                            pass

                    # --- CASCADE PRIORITY SELECTION ---
                    # 1. Slingshot
                    # 2. Cobra
                    # 3. GrindSnap
                    # 4. Others
                    ...
                    slingshot = next((s for s in tick_signals if s['algo'] == "Slingshot"), None)
                    cobra = next((s for s in tick_signals if s['algo'] == "Cobra"), None)
                    grind = next((s for s in tick_signals if s['algo'] == "GrindSnap"), None)
                    
                    if slingshot: 
                        selected_signal = slingshot
                    elif cobra: 
                        selected_signal = cobra
                    elif grind: 
                        selected_signal = grind
                    elif tick_signals: 
                        selected_signal = tick_signals[0] # First come first serve for others based on dict order
                    
                    if selected_signal:
                        name = selected_signal['algo']
                        res = selected_signal['res']
                        
                        direction = "UP" if "UP" in str(res) else "DOWN"
                        is_safe = (direction == "UP" and price > open_p) or (direction == "DOWN" and price < open_p)
                        
                        # Check against active trades - Prevent duplicate trades for same algo or conflicting directions if desired
                        # For Cascade, assuming we only want ONE active trade per window? Or multiple?
                        # User said "If Slingshot fires, ignore everything else". 
                        # This implies one priority signal PER TICK. 
                        # We also check if we already have an active trade for this algo in this window to prevent spam.
                        
                        already_active = any(t['algo'] == name for t in self.active_trades if t['window_start'] == window_start)
                        
                        if not already_active:
                            if is_safe: 
                                self.log_signal(name, res, price, elapsed, False, up_p if direction=="UP" else down_p)
                            else: 
                    ...
                    # Pending Signal Processing (Priority Override Check? No, pending logic flows separately)
                    # For simplicity, pending signals become "active" later. 
                    # If we really want Cascade to block Pendings too, we would filter here. 
                    # For now, leaving Pending logic as-is (it's weak confirmation anyway).
