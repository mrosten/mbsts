        ...
        try:
            ...
            if 10 <= elapsed <= 15 and not getattr(self, "audit_completed", False) and getattr(self, "audit_buffer", None):
                ...
                async def _perform_official_audit():
                    if not getattr(self, "app_active", False): return
                    try:
                        # 1. Fetch official P2B for the CURRENT window (This is the "Closing Price" of the previous window)
                        # Fresh swap-scrape to ensure we have the absolute baseline.
                        self.log_msg(f"[bold cyan]🔍 AUDIT INITIATED:[/bold cyan] Fetching Strike baseline from {current_slug}...", level="SYS")
                        current_data = await asyncio.to_thread(self.market_data_manager.fetch_polymarket, current_slug, force_refresh=True)
                        p2b_next = current_data.get("p2b") if current_data else None
                        ...
                        audit_src = "Poly"
                        if not p2b_next:
                            p2b_next = buf.get("close_price") # Fallback to Kraken Close recorded at T=0
                            audit_src = "Krak"
                        
                        if p2b_start and p2b_next:
                            # [SANITY CHECK] If Strike B vs Strike A delta is wild compared to Kraken, warn (Exchange drift)
                            k_delta = buf.get("close_price", 0) - buf.get("open_price", 0)
                            p_delta = p2b_next - p2b_start
                            drift = abs(p_delta - k_delta)
                            
                            if drift > 150:
                                self.log_msg(f"[yellow]⚠️ High Exchange Drift Detected: Poly Δ ${p_delta:,.2f} vs Local Δ ${k_delta:,.2f} (Drift: ${drift:,.2f})[/]", level="STATS")
                            
                            # Determine Official Winner based SOLELY on Polymarket Strikes (or Kraken fallback)
                            official_winner = "TIE"
                            ...
                            if official_winner == buf["pulse_result"]:
                                ...
                                # DIRECTIONAL MISMATCH (Exchange Drift flipped the result)
                            ...
                            try:
                                ...
                                with open(self.html_log_file, "a", encoding="utf-8") as f:
                                    ...
                                    f.write(f"<td>${p2b_start:,.2f}</td>") # Start Strike
                                    f.write(f"<td>${p2b_next:,.2f} ({audit_src})</td>")   # End Strike
