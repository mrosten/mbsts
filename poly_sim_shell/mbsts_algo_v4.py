...
class RiskManager:
    ...
    def calculate_bet_size(self, strategy_type, balance, consecutive_losses, trend_context):
        ...
        # This keeps bet sizes consistent within a single 5m window.
    ...
    def register_bet(self, cost):
        """
        Subtracts from available bankroll and tracks allocation for current window.
        """
...
class SniperApp(App):
        ...
        try:
            ...
            if self.market_data["start_ts"] != 0 and ts_start != self.market_data["start_ts"]:
                ...
                if total_payout > 0:
                    ...
                    # Even if no payout (loss or no bets), try to refill bankroll to target for the next window
                    self._add_risk_revenue(0)
            ...
            rem_min = max(1, min(5, (TradingConfig.WINDOW_SECONDS - elapsed + 59) // 60))
    ...
    def _add_risk_revenue(self, amount):
        ...
        # Enforce Refill/Capping Logic
        # We ensure the risk bankroll refills to its target amount from the main balance
        # if the balance allows. This keeps the bot's available betting capital consistent.
        is_live = self.query_one("#cb_live").value
        ...
        # Finally, cap at target to prevent exceeding the specified allocation
        if self.risk_manager.risk_bankroll > self.risk_manager.target_bankroll:
            self.risk_manager.risk_bankroll = self.risk_manager.target_bankroll
        ...
        if is_live:
             ...
             if token_id:
                ...
                if success: 
                    ...
                    self.window_bets[bet_id] = {"side": side, "entry": price, "cost": val}
